 ...
 *   data?: object    // optional key/value payload (values coerced to
 *                     // strings, as required by FCM)
 * }
 *
 ...
 * all of them, then removes any tokens that FCM reports as invalid or
 * unregistered from the owning user's document.
 *
 * Returns: {successCount, failureCount, invalidTokensRemoved}
 */
exports.sendNotification = onCall(async (request) => {
  // Require the caller to be authenticated. Remove this block if you
  // want to allow unauthenticated calls (not recommended).
  if (!request.auth) {
  ...
  if (invalidTokens.length > 0) {
    // Group invalid tokens by the user document they belong to, so
    // each document only needs a single update call.

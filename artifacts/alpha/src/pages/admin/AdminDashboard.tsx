...
const QUICK_ACTIONS = [
  { label: "Add Product", icon: PackagePlus, path: "/admin/products/new", color: "text-primary", bg: "bg-primary/10 border-primary/20", ring: "focus-visible:ring-primary/40" },
  { label: "Orders", icon: ClipboardList, path: "/admin/orders", color: "text-blue-400", bg: "bg-blue-400/10 border-blue-400/20", ring: "focus-visible:ring-blue-400/40" },
  { label: "Coupons", icon: Tag, path: "/admin/coupons", color: "text-amber-400", bg: "bg-amber-400/10 border-amber-400/20", ring: "focus-visible:ring-amber-400/40" },
  { label: "Users", icon: Users, path: "/admin/users", color: "text-violet-400", bg: "bg-violet-400/10 border-violet-400/20", ring: "focus-visible:ring-violet-400/40" },
  { label: "Products", icon: Package, path: "/admin/products", color: "text-teal-400", bg: "bg-teal-400/10 border-teal-400/20", ring: "focus-visible:ring-teal-400/40" },
  { label: "Settings", icon: Settings, path: "/admin/settings", color: "text-rose-400", bg: "bg-rose-400/10 border-rose-400/20", ring: "focus-visible:ring-rose-400/40" },
] as const;
...
/** Shared shimmer overlay used across every skeleton so loading states feel
 *  like one coherent system rather than several unrelated placeholders.
 *  Uses Tailwind's built-in `pulse` keyframe (via animate-pulse) so it works
 *  without any tailwind.config changes. */
function Shimmer() {
  return (
    <div className="absolute inset-0 animate-pulse bg-gradient-to-r from-transparent via-foreground/[0.05] to-transparent motion-reduce:hidden" />
  );
}
...
function StatCardSkeleton({ hero = false }: { hero?: boolean }) {
  return (
    <div
      className={`relative overflow-hidden flex flex-col gap-3 rounded-2xl bg-card border border-border/60 ${
        hero ? "p-5" : "p-4"
      }`}
      aria-hidden="true"
    >
...
function RowSkeleton() {
  return (
    <div
      className="relative overflow-hidden flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-card border border-border/60"
      aria-hidden="true"
    >
      <Shimmer />
...
function ChartSkeleton() {
  return (
    <div
      className="relative overflow-hidden flex flex-col gap-3 p-4 rounded-2xl bg-card border border-border/60"
      aria-hidden="true"
    >
      <Shimmer />
      <div className="h-4 w-40 rounded bg-muted/30" />
      <div className="h-32 rounded-xl bg-muted/15" />
    </div>
  );
}
...
function EmptyState({
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-2.5 py-11 rounded-2xl bg-muted/[0.15] border border-dashed border-border text-center px-4">
      ...
      {subtitle && <p className="text-xs text-muted-foreground max-w-[26ch] leading-relaxed">{subtitle}</p>}
    </div>
  );
}
...
/** Section heading used above every list/chart block for a consistent rhythm. */
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <div className="flex items-center gap-2">
        {Icon && <Icon className={`w-4 h-4 ${iconColor}`} aria-hidden="true" />}
...
export function AdminDashboard() {
  ...
  return (
    <div
      ref={scrollRef}
      className="relative flex flex-col gap-8 px-4 py-5 sm:px-6 overflow-y-auto"
    ...
    >
      ...
      <div
        className="absolute left-1/2 -translate-x-1/2 -top-12 flex items-center justify-center w-9 h-9 rounded-full bg-card border border-border shadow-md transition-opacity duration-200"
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
          {heroCards.map(({ key, label, value, icon: Icon, color, bg, glow, format }, i) => (
            <Reveal show={mounted} delayMs={i * 40} key={key}>
              <div
                className={`group relative overflow-hidden flex flex-col gap-3 p-5 rounded-2xl bg-card border border-border shadow-sm transition-all duration-200 active:scale-[0.97] hover:border-border hover:shadow-lg hover:-translate-y-0.5`}
                role="group"
                aria-label={`${label}: ${format === "currency" ? formatINR(value) : value.toLocaleString("en-IN")}`}
              >
                <div
                  className={`pointer-events-none absolute -right-6 -top-6 w-24 h-24 rounded-full bg-gradient-to-br ${glow} to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 blur-xl`}
                  aria-hidden="true"
                />
                <div className={`relative w-10 h-10 rounded-xl border flex items-center justify-center ${bg}`}>
      ...
      {loading ? (
        ...
        <Reveal show={mounted} delayMs={40}>
          <div className="relative overflow-hidden flex flex-col gap-4 p-5 rounded-2xl bg-gradient-to-br from-primary/10 via-card to-card border border-primary/20 shadow-sm">
            <div
              className="pointer-events-none absolute -right-10 -bottom-10 w-40 h-40 rounded-full bg-primary/10 blur-2xl"
              aria-hidden="true"
            />
            <div className="relative flex items-center justify-between">
              <SectionHeading icon={CalendarCheck} title="Today's Sales" />
            ...
            {stats.todayOrderCount === 0 ? (
              ...
              <div className="relative grid grid-cols-3 divide-x divide-border/70">
                <div className="flex flex-col gap-0.5 pr-3">
                  <span className="text-lg sm:text-xl font-bold text-primary tracking-tight">{formatINR(stats.todayRevenue)}</span>
                  <span className="text-[11px] text-muted-foreground">Revenue</span>
                </div>
                <div className="flex flex-col gap-0.5 px-3">
                  <span className="text-lg sm:text-xl font-bold tracking-tight">{stats.todayOrderCount}</span>
                  <span className="text-[11px] text-muted-foreground">Orders</span>
                </div>
                <div className="flex flex-col gap-0.5 pl-3">
                  <span className="text-lg sm:text-xl font-bold tracking-tight">{formatINR(stats.todayAvgOrderValue)}</span>
        ...
        <Reveal show={mounted} delayMs={80}>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
            {statCards.map(({ key, label, value, icon: Icon, color, bg, format }) => (
              <div
                key={key}
                className="flex flex-col gap-3 p-4 rounded-2xl bg-card border border-border shadow-sm transition-all duration-200 active:scale-[0.97] hover:border-border hover:shadow-md hover:-translate-y-0.5"
                role="group"
                aria-label={`${label}: ${format === "currency" ? formatINR(value) : value.toLocaleString("en-IN")}`}
              >
                ...
                <div className="flex flex-col gap-0.5">
                  <span className={`text-lg sm:text-xl font-bold tracking-tight ${color}`}>
                    {format === "currency" ? formatINR(value) : value.toLocaleString("en-IN")}
                  </span>
      ...
      {loading ? (
        ...
        <Reveal show={mounted} delayMs={120}>
          <div className="flex flex-col gap-4 p-4 sm:p-5 rounded-2xl bg-card border border-border shadow-sm">
            <SectionHeading icon={BarChart3} title="Revenue Overview (Last 7 Days)" />
            {stats.revenue === 0 ? (
              <EmptyState
                icon={BarChart3}
              ...
              >
                {revenueTrend.days.map((d, i) => {
                  ...
                  return (
                    <div key={i} className="group flex flex-col items-center gap-1.5 flex-1 h-full justify-end min-w-0">
                      <span className="text-[9px] text-muted-foreground truncate font-medium">
                        {d.total > 0 ? formatINR(d.total) : ""}
                      </span>
                      <div className="w-full h-full flex items-end">
                        <div
                          className="w-full rounded-lg bg-gradient-to-t from-primary to-primary/60 transition-all ease-out hover:from-primary hover:to-primary motion-reduce:transition-none"
                          style={{
      ...
      {loading ? (
        ...
        <Reveal show={mounted} delayMs={160}>
          <div className="flex flex-col gap-4 p-4 sm:p-5 rounded-2xl bg-card border border-border shadow-sm">
            <SectionHeading icon={PieChart} title="Order Status Summary" />
            {stats.orders === 0 ? (
              ...
              <div className="flex flex-col gap-3">
                {statusSummary.counts.map(({ status, count }) => (
                  <div key={status} className="flex items-center gap-3">
                    <span className="text-xs text-muted-foreground w-24 sm:w-28 shrink-0 truncate font-medium">
                      {statusLabel(status)}
                    </span>
                    ...
                    <span className="text-xs font-bold w-6 text-right shrink-0 tabular-nums">{count}</span>
                  </div>
                ))}
              </div>
            )}
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-3 sm:grid-cols-6 gap-3">
          {QUICK_ACTIONS.map(({ label, icon: Icon, path, color, bg, ring }) => (
            <button
              key={label}
              type="button"
              onClick={() => handleQuickAction(path)}
              aria-label={`Open ${label}`}
              className={`group flex flex-col items-center gap-2 p-4 rounded-2xl bg-card border border-border shadow-sm transition-all duration-200 ease-out active:scale-90 hover:border-primary/40 hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 ${ring}`}
            >
              <div
                className={`w-10 h-10 rounded-xl border flex items-center justify-center transition-transform duration-200 group-hover:scale-110 group-active:scale-95 ${bg}`}
              >
                <Icon className={`w-[18px] h-[18px] ${color}`} aria-hidden="true" />
              </div>
        ...
        <SectionHeading
          ...
          action={
            <button
              ...
              className="flex items-center gap-0.5 text-xs text-primary font-semibold hover:underline underline-offset-2"
            >
            ...
            {recentOrders.map((order) => {
              ...
              return (
                <button
                  ...
                  className="flex items-center gap-3 px-4 py-3.5 rounded-2xl bg-card border border-border shadow-sm text-left transition-all duration-200 active:scale-[0.98] hover:border-primary/30 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/40"
                >
                  <div
                    className={`w-9 h-9 rounded-xl shrink-0 flex items-center justify-center ${statusColor(order.status)}`}
                    aria-hidden="true"
                  >
                  ...
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <span className="text-sm font-bold text-primary tracking-tight">{formatINR(total)}</span>
      ...
      {/* Low Stock */}
      <div className="flex flex-col gap-3">
        <SectionHeading icon={AlertTriangle} iconColor="text-rose-400" title="Low Stock (below 10)" />
        {loading ? (
          <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            {lowStockProducts.map((product) => {
              ...
              return (
                <div
                  key={product.id}
                  className={`flex items-center gap-3 px-4 py-3 rounded-2xl bg-card border shadow-sm transition-colors duration-200 ${
                    critical ? "border-rose-400/30" : "border-rose-400/15"
                  }`}
                >
                  {product.imageUrl ? (
                    <img
                      ...
                      alt={name}
                      className="w-11 h-11 rounded-xl object-cover shrink-0 border border-border/50"
                      loading="lazy"
                    />
                  ) : (
                    <div
                      className="w-11 h-11 rounded-xl shrink-0 border border-border/50"
                      role="img"
                  ...
                  <span
                    className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ${
                      critical ? "text-rose-400 bg-rose-400/15" : "text-rose-400 bg-rose-400/10"
                    }`}
                    aria-label={`${stock} left in stock`}
                  >
                    {critical && <AlertTriangle className="w-2.5 h-2.5" aria-hidden="true" />}
                    {stock} left
                  </span>
                </div>
              );
      ...
      <div className="flex flex-col gap-3 pb-2">
        <SectionHeading icon={TrendingUp} title="Top Selling Products" />
        {loading ? (
          <div className="flex flex-col gap-2">
            {Array.from({ length: 3 }).map((_, i) => (
              <RowSkeleton key={i} />
          ...
          <div className="flex flex-col gap-2">
            {topProducts.map((product, idx) => {
              ...
              return (
                <div
                  key={product.id}
                  className="flex items-center gap-3 px-4 py-3 rounded-2xl bg-card border border-border shadow-sm transition-colors duration-200 hover:border-border"
                >
                  <span
                    className={`flex items-center justify-center text-[11px] font-bold w-6 h-6 rounded-full shrink-0 ${
                      idx === 0
